...
class MyMongodb:
    ...
    def insert_more(self, db_name, data):
        ...
        print(f"{count} element exist insert false")
    ...
    def get_projects(self,db_name):
        collection = self.select_collection(db_name)
        data= list(collection.find({}, {'_id': 0,'name': 1, 'description':1,'gallery':1}))
        return data
    ...
    #更新数据
    def update_data(self, db_name, data, new_data):
        collection = self.select_collection(db_name)
        return collection.update_many(data,{"$set":new_data})
